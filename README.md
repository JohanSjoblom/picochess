PicoChess
=========
PicoChess is chess engine management software for Raspberry Pi and other Linux computers; it is not itself a chess engine. Its distinctive strength is combining modern and retro chess engines with physical electronic chess boards. Retro chess computers can be emulated through MAME.

PicoChess can also be used without an electronic board: play entirely from a web browser on a phone, tablet, or desktop. Installation includes Stockfish and Leela Chess Zero (LC0) as examples, and you can add other modern or retro engines, including classics such as Mephisto and TuroChamp. See the "How to add more engines" section below for details.

[♟️ Support AI-assisted PicoChess development](https://paypal.me/picochess)

Contributions help cover the AI service subscription costs used for PicoChess development and maintenance; they are not intended as payment for the maintainers' work hours.
At present, these services cost approximately US$24 per month. Future use of automated development agents on GitHub may add further service costs.
Companies interested in sponsoring these monthly costs are welcome to contact the maintainers through the [PicoChess Google Group](https://groups.google.com/g/picochess).
...
- Use phone browser audio as an alternative to local speakers. This is useful on headless systems such as DGT3000 clock setups or Pi systems without external speakers.
...
For headless setups (for example DGT3000), you can use your phone browser as both display and audio output instead of local speakers.
...
----------------------
Install the Fairy-Stockfish 14 engine along with fairy.uci and enable it in engines.ini to play some fun chess variants. Five variants are implemented as engine levels, each with four Elo strengths (1000, 1500, 2000, 2850):
...
5. Antichess: the player who loses all his pieces first or gets stalemated, wins.
